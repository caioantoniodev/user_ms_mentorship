...
public class UserService implements UserServicePortIn {
    ...
    private void merge(Map<String, Object> requestData, UserEntity userEntity) {
        ...
        requestData.forEach((propertyName, propertyValue) -> {
            var foundedField = ReflectionUtils.findField(UserEntity.class, propertyName);
            isAllowedField(foundedField);
            foundedField.setAccessible(true);

            var foundedValue = ReflectionUtils.getField(foundedField, userRequest);
            ReflectionUtils.setField(foundedField, userEntity, foundedValue);
        });
    }
